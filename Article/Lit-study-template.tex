...
`\subsection{Another Problem Formulation}

We want to reformulate the following problem from the course: 

\begin{equation*}
    \min_{x\in \mathbb R^n} \sum_{i=1}^N f_i(x)
\end{equation*}

Donc en repennant le formalisme de l'article on obtient dans ce cas le problème reformulé suivant :

\begin{equation*}
    \begin{aligned}
    &\min_{x^i\in\mathbb{R}^{n}, y^{ij}\in\mathbb{R}^{n}} \sum_{i=1}^{N} f_i(x^i)\\
    \text{s.t } & y^{ij}=\zeta_{j} \quad \forall i\sim j & \textit{(neighboring constraint)}\\ 
    & x^i=\zeta_i \quad \forall i \in M & \textit{(local constraint)}\\
    & x^i=y^{ij} \quad \forall i\sim j & \textit{(local constraint)}
    \end{aligned}
\end{equation*}

\textcolor{red}{Note: ici les $z_i$ sont $(x^i, y^{ij} \quad \forall j \sim i)$, et il n'y a pas de     $F_i$, $p_i$}

The problem formulation consists in finding a peer-to-peer algorithm which fulfilled the following as-
sumptions:

\begin{enumerate}
    \item Only agent $i$ should have access to $x^i,f_i $.
    \item  Neither the agents nor the operator should have access to plaintext values of any $y^{ij}$.        
    \item Neither the agents nor the operator should have access to plaintext values of $\zeta$, with the exception that agent i may have access to those entries reflecting $x^i$.
\end{enumerate}'

Once we saw how to link the problem from the course to the one in the article
